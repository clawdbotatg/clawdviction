...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (wallet?.toLowerCase() === ADMIN_WALLET) {
      // Admin: full response list
      const responses = await sql`
        SELECT wallet, response, reasoning, created_at FROM governance_responses
        WHERE proposal_id = ${id} ORDER BY created_at ASC`;

      // Vote tallies for vote type
      let tallies = null;
      if (proposal.type === "vote") {
        const tallyResult = await sql`
          SELECT response, COUNT(*)::int as count FROM governance_responses
          WHERE proposal_id = ${id} GROUP BY response`;
        tallies = { yes: 0, no: 0, abstain: 0 };
        for (const row of tallyResult.rows) {
          const key = row.response.toLowerCase().trim();
      ...
      const userResponse = await sql`
        SELECT response, reasoning, created_at FROM governance_responses
